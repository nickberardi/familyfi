...
export async function GET(request: Request) {
  return withAdmin(request, async () => {
    const devices = await prisma().pairedDevice.findMany({ orderBy: { lastSeenAt: "desc" }, include: { edgeTokens: { select: { endpointId: true, version: true } }, pairings: { include: { endpoint: true }, take: 1 }, sessions: { where: { revokedAt: null, expiresAt: { gt: new Date() } }, select: { id: true, username: true, expiresAt: true, createdAt: true } } } });
    return Response.json({ devices: devices.map((device) => ({ id: device.id, displayName: device.displayName, client: device.client, enrolledAt: device.createdAt.toISOString(), lastSeenAt: device.lastSeenAt?.toISOString() ?? null, revokedAt: device.revokedAt?.toISOString() ?? null, pairedVia: pairedVia(device.pairings[0]?.endpoint ?? null), edgeTokens: device.edgeTokens, sessions: device.sessions.map((session) => ({ id: session.id, username: session.username, client: device.client, expiresAt: session.expiresAt.toISOString(), createdAt: session.createdAt.toISOString() })) })) });
  });
}
