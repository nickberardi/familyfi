...
- Remote access tunnels point only at the phone-only gateway (`src/server/tunnel/phone-gateway.ts`), never at the web app. The gateway passes `/api/v1/*` only, strips cookies, and stamps `x-familyfi-via: tunnel`; over a tunnel, sign-in requires a paired phone checked before the password. `cloudflared` is pinned and hash-checked in the image, never downloaded at runtime. *Tests:* `tests/unit/phone-gateway.test.ts`, `tests/integration/remote-access.test.ts`, `tests/integration/connection-api.test.ts`.
